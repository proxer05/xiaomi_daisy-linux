#!/usr/bin/env bash
#
# build.sh — Build an Arch Linux ARM image for Xiaomi Mi A2 Lite (daisy/MSM8953)
#
# Based on:
#   - https://github.com/umeiko/KlipperPhonesLinux/ (build process)
#   - https://gitlab.postmarketos.org/postmarketOS/pmaports (firmware-qcom-msm8953)
#   - PostmarketOS msm8953 / xiaomi-daisy device support (kernel, DTB, lk2nd)
#
# Prerequisites:
...
#   --with-plasma      Install KDE Plasma Mobile (WIP)
...
#   - flash_archlinux.sh (convenience script to flash everything)
#
set -euo pipefail
IFS=$'\n\t'
...
SOC="msm8953"
# DTB families to include (matches deviceinfo_dtb from postmarketOS qcom-msm8953)
...
# Firmware (from postmarketOS firmware-qcom-msm8953 / firmware-xiaomi-daisy)
# Pinned commit from: https://gitlab.postmarketos.org/postmarketOS/pmaports/-/blob/main/device/community/firmware-qcom-msm8953/APKBUILD
...
ALARM_ROOTFS_ARCHIVE="${WORK_DIR}/ArchLinuxARM-aarch64-latest.tar.gz"
...
prepare_kernel() {
    ...
    # The downloaded config was generated with Alpine Clang.
    # Run olddefconfig so Kbuild can adapt it to our GCC cross-compiler.
    # Force display and touch drivers to be built-in so we can see the boot logs!
    local configs=(
        "CONFIG_DRM_MSM"
    ...
    # Force Qualcomm Wi-Fi driver to be built as a module (required for WCN3990)
    local configs_m=(
        "CONFIG_ATH10K"
        "CONFIG_ATH10K_SNOC"
    )
...
compile_kernel() {
    ...
    # Concatenate matching DTBs to allow lk2nd to dynamically select the best match
...
###############################################################################
# Step 5: Fetch firmware blobs (postmarketOS firmware-xiaomi-daisy)
###############################################################################
fetch_firmware() {
...
###############################################################################
# Step 6: Create Arch Linux ARM rootfs
###############################################################################
create_rootfs() {
    log "Creating Arch Linux ARM rootfs (${ROOTFS_SIZE_MB} MB)..."

    # Download ALARM tarball if needed
    if [[ ! -f "${ALARM_ROOTFS_ARCHIVE}" ]]; then
        log "Downloading Arch Linux ARM aarch64 rootfs..."
        wget -q --show-progress -O "${ALARM_ROOTFS_ARCHIVE}" "${ALARM_ROOTFS_URL}"
    fi

    # Create ext4 image
    log "Creating ext4 image: ${ROOTFS_IMG}"
    dd if=/dev/zero of="${ROOTFS_IMG}" bs=1M count="${ROOTFS_SIZE_MB}" status=progress
    mkfs.ext4 -F -L "archlinux" "${ROOTFS_IMG}"
    ...
    # Extract ALARM rootfs
    log "Extracting Arch Linux ARM rootfs..."
    if command -v bsdtar &>/dev/null; then
        bsdtar -xpf "${ALARM_ROOTFS_ARCHIVE}" -C "${ROOTFS_MOUNT}"
    else
        tar -xpf "${ALARM_ROOTFS_ARCHIVE}" -C "${ROOTFS_MOUNT}"
    fi
    ...
    # Install firmware (postmarketOS firmware-xiaomi-daisy)
    ...
    # GPU: Adreno 506 zap shader → /lib/firmware/qcom/msm8953/xiaomi/daisy/
    ...
    # WCNSS: remoteproc firmware (wcnss.mdt, wcnss.b*) → /lib/firmware/
    #         WiFi NV calibration (WCNSS_qcom_wlan_nv.bin) → /lib/firmware/wlan/prima/
    if [[ -d "${FW_DAISY_DIR}/wcnss" ]]; then
        # Remoteproc PIL blobs go flat in /lib/firmware/
        for blob in "${FW_DAISY_DIR}/wcnss"/wcnss.{mdt,b*}; do
            [[ -f "${blob}" ]] || continue
            install -Dm644 "${blob}" "${fw_base}/$(basename "${blob}")"
        done
        log "  Installed WCNSS remoteproc firmware (wcnss.mdt + segments) → ${fw_base}/"

        # WiFi NV calibration goes to /lib/firmware/wlan/prima/
        if [[ -f "${FW_DAISY_DIR}/wcnss/WCNSS_qcom_wlan_nv.bin" ]]; then
            ...
            log "  Installed WiFi NV calibration → ${fw_base}/wlan/prima/WCNSS_qcom_wlan_nv.bin"
        else
            warn "WCNSS_qcom_wlan_nv.bin not found in ${FW_DAISY_DIR}/wcnss"
        fi
    else
        warn "WCNSS firmware not found in ${FW_DAISY_DIR}/wcnss"
    fi

    # ADSP: remoteproc firmware (adsp.mdt, adsp.b*) → /lib/firmware/
    ...
    # Modem: remoteproc firmware (modem.mdt, modem.b*, mba.mbn) → /lib/firmware/
    ...
    # Also install from host linux-firmware if available (fallback)
    ...
    "${SCRIPT_DIR}/scripts/configure_rootfs.sh" "${ROOTFS_MOUNT}"
    
    # Copy PKGBUILDs into rootfs
    if [[ -d "${SCRIPT_DIR}/pkgbuilds" ]]; then
        log "Copying local PKGBUILDs to rootfs..."
        cp -a "${SCRIPT_DIR}/pkgbuilds" "${ROOTFS_MOUNT}/opt/pkgbuilds"
    fi
    ...
    # Copy qemu for chroot on x86_64 host
    if [[ "$(uname -m)" == "x86_64" ]]; then
        ...
        warn "qemu-aarch64-static not found; chroot may fail on x86_64 host"
    fi

    # Set up DNS for chroot network access
    local resolv_backup=""
    if [[ -f "${ROOTFS_MOUNT}/etc/resolv.conf" ]]; then
        resolv_backup="$(cat "${ROOTFS_MOUNT}/etc/resolv.conf")"
    fi
    # Create a temporary resolv.conf with public DNS servers
    echo "nameserver 8.8.8.8" > "${ROOTFS_MOUNT}/etc/resolv.conf"
    echo "nameserver 1.1.1.1" >> "${ROOTFS_MOUNT}/etc/resolv.conf"
    if [[ -f /etc/resolv.conf ]]; then
        # Also append the host's DNS servers as backup
        grep -v '^#' /etc/resolv.conf | grep 'nameserver' >> "${ROOTFS_MOUNT}/etc/resolv.conf" || true
    fi

    # Run chroot setup script
    cp "${SCRIPT_DIR}/scripts/chroot_setup.sh" "${ROOTFS_MOUNT}/tmp/"
    chroot "${ROOTFS_MOUNT}" /bin/bash /tmp/chroot_setup.sh "${WITH_PLASMA}"
    rm -f "${ROOTFS_MOUNT}/tmp/chroot_setup.sh"

    # Restore resolv.conf or create systemd-resolved symlink on the target rootfs
    rm -f "${ROOTFS_MOUNT}/etc/resolv.conf"
    if [[ -n "${resolv_backup}" ]]; then
        echo "${resolv_backup}" > "${ROOTFS_MOUNT}/etc/resolv.conf"
    else
        # Standard systemd-resolved stub resolv symlink for Arch
...
generate_initramfs() {
    ...
    # Instead of a fragile custom init script, we will provide an EMPTY initramfs.
    # When the Linux kernel finds no `/init` script, it natively handles mounting
    # the root filesystem using our cmdline: root=PARTLABEL=userdata rootwait
...
create_boot_img() {
    ...
    # Concatenate kernel + DTB (appended DTB format used by MSM8953)
    ...
    if command -v mkbootimg &>/dev/null; then
        ...
        # Try python mkbootimg from Android
...
package_output() {
    ...
    # Create flash script
    cat > "${OUTPUT_DIR}/flash_archlinux.sh" << 'FLASH_EOF'
#!/usr/bin/env bash
#
# flash_archlinux.sh — Flash Arch Linux ARM to Xiaomi Mi A2 Lite (daisy)
#
# Prerequisites:
...
echo "=============================================="
echo " Arch Linux ARM Flasher for Mi A2 Lite (daisy)"
echo "=============================================="
echo ""
...
echo " Connect via USB networking or WiFi."
echo "=============================================="
FLASH_EOF
    chmod +x "${OUTPUT_DIR}/flash_archlinux.sh"
...
main() {
    ...
    log "======================================================="
    log " Arch Linux ARM Image Builder"
    ...
    if [[ "${SKIP_KERNEL}" == false ]]; then
        ...
        # Concatenate matching DTBs (apq8053/msm8953/sdm450)
    ...
    log "   ${OUTPUT_DIR}/root.img     - Arch Linux ARM rootfs"
    [[ -n "${LK2ND_IMAGE:-}" ]] && \
    log "   ${OUTPUT_DIR}/lk2nd.img    - Secondary bootloader"
    log "   ${OUTPUT_DIR}/flash_archlinux.sh - Flash script"
    log ""
    log " To flash:"
    log "   cd ${OUTPUT_DIR} && sudo ./flash_archlinux.sh"
    log "======================================================="
}
